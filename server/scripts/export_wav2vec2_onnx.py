...
def main():
    ...
    parser.add_argument("--opset", type=int, default=17,
                        help="ONNX opset to export with (default 17 — the last opset tied to "
                             "ir_version 8, which onnxruntime-web's WASM/WebGPU builds parse "
                             "reliably; newer opset/ir_version combos have been seen tripping "
                             "'protobuf parsing failed' errors in the browser even though "
                             "native onnxruntime loads them fine)")
    parser.add_argument("--ir-version", type=int, default=8,
                        help="ONNX IR version to stamp on the exported model, overriding "
                             "whatever the local onnx package defaults to (see --opset)")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if args.ir_version:
            import onnx
            print(f"Downgrading ir_version to {args.ir_version} for browser compatibility...")
            fp32_model = onnx.load(str(onnx_dir / "model.onnx"))
            fp32_model.ir_version = args.ir_version
            onnx.save(fp32_model, str(onnx_dir / "model.onnx"))
